...
def generate_launch_description():
    ...
    declare_path_map1 = DeclareLaunchArgument(
        'map1_file',
        default_value=os.path.join(get_package_share_directory('edu_robocup_rescue_stack'), 'map', 'map_6.1.yaml'),
        )

    declare_path_map2 = DeclareLaunchArgument(
        'map2_file',
        default_value=os.path.join(get_package_share_directory('edu_robocup_rescue_stack'), 'map', 'map_6.2.yaml'),
        )
    
    declare_path_map3 = DeclareLaunchArgument(
        'map3_file',
        default_value=os.path.join(get_package_share_directory('edu_robocup_rescue_stack'), 'map', 'map_6.3.yaml'),
        )
    
    declare_path_map4 = DeclareLaunchArgument(
        'map4_file',
        default_value=os.path.join(get_package_share_directory('edu_robocup_rescue_stack'), 'mao', 'map_6.4.yaml'),
        )
    ...
    static_transform_publisher_base_frame_laser_link_node = Node(
            ...
            arguments = ['--x', '0.1', '--y', '0', '--z', '0.16', '--yaw', '-0', '--pitch', '0', '--roll', '0', '--frame-id', 'base_frame', '--child-frame-id', 'laser_link']        )
