...
void Control::calculateAngleSpeed() {
    ...
    if(m_slowDownTurning > 1){
        ...
        m_slowDownTurning = m_slowDownTurning - 5;
        if(m_slowDownTurning < 1){
            m_slowDownTurning = 1;
        }
    }
    ...
    if((delta_phi > m_dest_precision) || (delta_phi < -m_dest_precision)) {
        m_speed = 0.001;
        m_slowDownSpeed = 200;
        m_slowDownSpeedReduce = 20;
    } 
    else {
        //slow Down calculation
        m_slowDownSpeed = m_slowDownSpeed - m_slowDownSpeedReduce;
        if( m_slowDownSpeed < 20){
            m_slowDownSpeed = 20;
        }
        // speed calculation
        m_speed = (delta_dest) / (float) m_slowDownSpeed;
        if(m_speed < 0.3){
            m_speed  = 0.3;   
        }
    } 
...
void Control::setNaviagtionStep(){
    ...
    if( m_navigationStep == 0 ){
        ...
        //slows Roboter down
        m_slowDownSpeed = 100;
        m_slowDownSpeedReduce = 2;
        m_slowDownTurning = 20;
        //switches map
